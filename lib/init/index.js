...
async function getQuestion (
  question,
  { type = 'confirm', def = 'output', choices = [] }
) {
...
async function init (option, configFile) {
  try {
    ...
    downloadFunc(
      'direct:https://github.com/kongnet/skybase-template.git',
      downloadDir,
      { clone: true },
      function (err) {
    ...
    commandObj.proName = await getQuestion(
      '请输入您的项目名称 [skybase-test]: ',
      { type: 'input', def: 'skybase-test' }
    )
    ...
    commandObj.proVersion = await getQuestion(
      '请输入您的项目版本号 [0.1.0]: ',
      { type: 'input', def: '0.1.0' }
    )
    const dateStr = new Date().date2Str()
      .split(' ')[0]
    commandObj.proDesc = await getQuestion(
      `请输入您的项目详细说明 [我的项目${dateStr}]: `,
      { type: 'input', def: `我的项目${dateStr}` }
    )
    ...
    $.file.deleteAll(`./${downloadDir}`)
    $.log('========请继续运行如下命令========\n')
    $.log($.c.y(`    cd ./${projectName}`))
    $.log($.c.y('    npm i'))
    $.log(
      $.c.y('    nodemon '),
      $.c.dimw('// 如果没有安装请全局安装 npm i -g nodemon')
    )
    $.log(
      $.c.y('    http://127.0.0.1:13000/skyapi/mock/first'),
      $.c.dimw('打开浏览器可访问')
    )
    $.log(
      $.c.dimw(`    // 数据的配置在 ./${projectName}/config/config.default.js`)
    )
    $.log(
      $.c.y('    http://127.0.0.1:13000/skyapi/probe/mysql'),
      $.c.dimw('//正确配置数据库设置后可访问'),
      '\n'
    )
    $.log('============祝您开心============')
...
async function initMp (option, configFile) {
  try {
    let {
      name: projectName,
      force: isForceCover,
    ...
    $.log($.c.c('请耐心等待正在下载MP模板...'))
    downloadFunc(
      'direct:https://github.com/xueyan1/miniprogram-template.git#develop',
      downloadDir,
      { clone: true },
      function (err) {
        let status = err ? 'Error' : 'Success'
    ...
    $.log('模板下载 ', downObj.status)
    if (downObj.status === 'Error') {
      $.log('请尝试删除当前目录下的 _temp_jt_mp 目录后再次安装！')
      process.exit(0)
    }

    commandObj.proName = await getQuestion('请输入您的项目名称 [mp-test]: ', {
      type: 'input',
      def: 'mp-test'
    })
    projectName = commandObj.proName || 'mp-test'
    let inObj = {}

    const settingObj = require(`${process.cwd()}/${downloadDir}/mpOutFile.js`)(
      commandObj
    )
    inObj[projectName] = settingObj
    gen.genMain(inObj, baseDir, { templateDir: '.', open: '<%', close: '%>' })
    $.file.deleteAll(`./${downloadDir}`)
    $.log('小程序项目已经初始化完成！')
