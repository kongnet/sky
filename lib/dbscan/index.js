...
async function getDB (conf) { // 获取全部 db 列表或者 conf 设置中的db列表
  let r = conf.checkDB.length > 0 ? conf.checkDB : (await db.cmd('show databases;').run())[1]
  let sql = ''
  return r
}
async function getTable (dbName) { // 获取全部 db 中所有table列表
...
function fieldEnum (field) {
  let b1 = (/^(stat|type|enum)_/m).test(field.Field) && (!field.Type.includes('tinyint') || !(/\[.+\]/m).test(field.Comment) || !field.Type.includes('varchar'))
  let b2 = (/_(stat|type|enum)$/m).test(field.Field) && (!field.Type.includes('tinyint') || !(/\[.+\]/m).test(field.Comment) || !field.Type.includes('varchar'))

  let b3 = (/\[.+\]/m).test(field.Comment) && (!(/^(stat|type|enum)_/m).test(field.Field) || !field.Type.includes('tinyint') || !field.Type.includes('varchar'))
  let b4 = (/\[.+\]/m).test(field.Comment) && (!(/_(stat|type|enum)$/m).test(field.Field) || !field.Type.includes('tinyint') || !field.Type.includes('varchar'))
  return b1 || b2 || b3 || b4
}
async function scan (file) {
  try {
    // console.log(path.join(process.cwd(), file))
    conf = file ? require(path.join(process.cwd(), file)).dbscan : require('./config').dbscan
  } catch (e) {
