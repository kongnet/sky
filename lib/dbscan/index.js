...
let fs = require('fs')
let confDefault = require('./config')
let conf
let db
...
async function getDB (conf) {
  if (!conf.checkDB) {
    console.error($.c.r('缺少dbscan.checkDB字段！'))
    return []
  }
  let r =
    conf.checkDB.length > 0
      ? conf.checkDB
      : (await db.cmd('show databases;').run())[1]
  return r
}
// 获取全部 db 中所有table列表
...
function ifPlural (str) {
  return (
    /s$/m.test(str) &&
    !conf.excludePlural.some(function (it) {
      return str.includes(it)
    })
  )
}
...
function noDefaultValue (field) {
  return (
    !conf.exDefaultType.some(function (it) {
...
function fieldIncludeIs (field) {
  let b1 =
    /^is_/m.test(field.Field) &&
    (!field.Type.includes('tinyint') || !field.Comment.includes('是否'))
  let b2 =
    field.Comment.includes('是否') &&
    (!/^is_/m.test(field.Field) || !field.Type.includes('tinyint'))
  return b1 || b2
}
// 枚举型字段的判断
function fieldEnum (field) {
  let isHead = /^(stat|type|enum|status)_/gm.test(field.Field)
  let isRear = /_(stat|type|enum|status)$/gm.test(field.Field)
...
async function scan (file, jsonObj) {
  const errGroup = {
    Lev1: {
      ...
      '0x90': [
        '注解含有[xxx],为枚举，必须以(enum/stat/type/status)_xxx开头或结尾,数据类型tinyint或varchar,反之依然'
      ],
  ...
  try {
    ...
    conf =
      jsonObj ||
      (file
        ? require(path.join(process.cwd(), file)).dbscan
        : fs.existsSync('./sky-db-scan-config.js')
        ? require(path.join(process.cwd(), './sky-db-scan-config.js')).dbscan
        : require('./config').dbscan)
  } catch (e) {
    console.log(e)
  ...
  conf.excludeWord = conf.excludeWord || confDefault.dbscan.excludeWord
  conf.exDefaultType = conf.exDefaultType || confDefault.dbscan.exDefaultType
  conf.excludePlural = conf.excludePlural || confDefault.dbscan.excludePlural
    ...
    for (let k = 0; k < tableObj.length; k++) {
      ...
      if (tableItem.comment.length < 1) {
        errGroup.Lev1['0x10'].push(
          [dbItem, tableItem.name].join(' -> ').fillStr(' ', 100)
        )
      }
      ...
      for (let m = 0; m < fieldObj.length; m++) {
        ...
        if (ifNoComment(fieldItem)) {
          errGroup.Lev1['0x10'].push(
            [dbItem, tableItem.name, fieldItem.Field]
              .join(' -> ')
              .fillStr(' ', 100)
          )
        }
        ...
        if (ifDecimal(fieldItem.Type)) {
          errGroup.Lev1['0x70'].push(
            [dbItem, tableItem.name, fieldItem.Field, fieldItem.Type]
              .join(' -> ')
              .fillStr(' ', 100)
          )
        }
        if (fieldIncludeIs(fieldItem)) {
          errGroup.Lev1['0x80'].push(
            [
              dbItem,
              tableItem.name,
              fieldItem.Field,
        ...
        if (noDefaultValue(fieldItem)) {
          // $.dir(fieldItem)
          errGroup.Lev1['0xA0'].push(
            [
              dbItem,
              tableItem.name,
              fieldItem.Field,
              fieldItem.Type,
              fieldItem.Default || '[Null]'
            ]
              .join(' -> ')
              .fillStr(' ', 100)
          )
...
function genJson () {
  fs.writeFileSync(
    path.join(process.cwd(), 'sky-db-scan-config.js'),
    `const conf = ${JSON.stringify(confDefault, null, 2)}
    module.exports = conf`
  )
  console.log(
    $.c.y('sky-db-scan-config.js'),
    '已经生成，请修改对应db，此文件建议加入忽略名单'
  )
  console.log('运行', $.c.y('dn db -c sky-db-scan-config.js'))
}
module.exports = { scan, genJson }
