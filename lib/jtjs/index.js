...
let regDefined = function () {
  return {
    r1: new RegExp(`/\\d+/g`)
  }
}
/*
let func2Str = function (obj) {
  let rObj = {}
  let objWalk = function (itemObj, initObj = {}) {
    let rWalkObj = initObj || {}
    for (let i in itemObj) {
      if (typeof itemObj[i] === 'object') {
        rObj[i] = {}
        objWalk(itemObj[i], rObj[i])
      } else {
        if (typeof itemObj[i] === 'function') {
          rWalkObj[i] = `new Function('${itemObj[i].toString()}')`
        }
      }
    }
  }
...
function writeFile (configPath) {
  try {
    ...
    console.log('耐心等待eslint格式化...')

    execSync(`npm run eslintfix`)
  } catch (e) {
    if (e.output) console.error(e.output.toString())
  }
