...
function gitStat () {
  try {
    ...
    r = execSync("git --no-pager log --pretty='%ae$%an' --no-merges || true", {
      timeout: timeout
    })
    ...
    for (let i in r2) {
      ...
      let str = `git --no-pager log --author="${iAry[0] ||
      ...
      tableData.push({
        email: iAry[0] || 'Unknown',
        name: (iAry[1] || '').split("'")[0] || 'Unknown',
        commits: r2[i],
        '+ Lines': sumAdd,
        '- Lines': sumMinus,
        '* diff': sumAdd - sumMinus,
        percent: ''
      })
    }
    tableData = tableData.map(item => {
      ...
      return {
        ...
        name: item.name,
        commits:
          ' '.times(colWidth[2] - (item.commits + '').length) + item.commits,
        '+ Lines':
          ' '.times(colWidth[3] - (item['+ Lines'] + '').length) +
          item['+ Lines'],
        '- Lines':
          ' '.times(colWidth[4] - (item['- Lines'] + '').length) +
          item['- Lines'],
        '* diff':
          ' '.times(colWidth[5] - (item['* diff'] + '').length) +
          item['* diff'],
    ...
    console.log(
      diffTime < 1000
        ? diffTime + ' 毫秒'
        : $.tools.timeAgo(t, Date.now()).split('前')[0]
    )
  } catch (e) {
    $.err(e.toString())
  }
