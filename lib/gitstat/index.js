...
function gitStat () {
  try {
    ...
    for (let i in r2) {
      ...
      i = i.replaceAll('\'', '')
      let str = `git --no-pager log --author="${i.split('$')[0] || ''}" --pretty=tformat: --numstat`
      ...
      tableData.push({ 'email': i.split('$')[0] || 'Unknown', 'name': (i.split('$')[1] || '').split('\'')[0] || 'Unknown', 'commits': r2[i], '+ Lines': sumAdd, '- Lines': sumMinus, '* diff': sumAdd - sumMinus, percent: '' })
    }
