  ...
  if (checkServData) {
    ...
    if (checkServData.title && checkServData.title.trim().length > 0) {
      if (utils.isValidChinese(checkServData.title)) {
        ...
        checkArr.push(false)
        errGroup.lev1['0x10'].push(
          `服务必须有中文名  --->  ${checkServData.title || null}`
        )
      }
    } else {
      ...
      checkArr.push(false)
      errGroup.lev1['0x10'].push(
        `服务必须有中文名且不能为空  --->  ${checkServData.title || null}`
      )
    }

    // 服务中文描述检测
    if (
      checkServData.description &&
      checkServData.description.trim().length > 0
    ) {
      if (utils.isValidChinese(checkServData.description)) {
        ...
        checkArr.push(false)
        errGroup.lev1['0x10'].push(
          `服务必须有中文描述  --->  ${checkServData.description || null}`
        )
      }
    } else {
      ...
      checkArr.push(false)
      errGroup.lev1['0x10'].push(
        `服务必须有中文描述且不能为空  --->  ${checkServData.description ||
          null}`
      )
    }
    ...
    // 服务联系人姓名检测
    if (
      checkServData.contact &&
      checkServData.contact.name &&
      checkServData.contact.name.trim().length > 0
    ) {
      // console.log('\x1b[32m', '✔  服务存在联系人姓名  --->  ' + `${checkServData.contact.name}`, '\x1b[0m')
      checkArr.push(true)
    } else {
      const contactName =
        checkServData.contact && checkServData.contact.name || null
      // console.log('\x1b[31m', '✘  服务必须有联系人姓名且不能为空  --->  ' + `${contactName}`, '\x1b[0m')
      checkArr.push(false)
      errGroup.lev1['0x10'].push(
        `服务必须有联系人姓名且不能为空  --->  ${contactName}`
      )
    }
    ...
    if (
      checkServData.contact &&
      checkServData.contact.email &&
      checkServData.contact.email.trim().length > 0
    ) {
      if (utils.validataEmail(checkServData.contact.email)) {
        ...
        checkArr.push(false)
        errGroup.lev1['0x10'].push(
          `服务存在联系人邮箱异常  --->  ${checkServData.contact.email || null}`
        )
      }
    } else {
  ...
  for (let key in checkApiDescData) {
    if (checkApiDescData[key].get) {
      if (utils.isValidChinese(checkApiDescData[key].get.summary)) {
        ...
        checkArr.push(false)
        errGroup.lev1['0x20'].push(
          `接口[ GET  -> ${key} ]中必须有中文介绍描述  --->  ${checkApiDescData[
            key
          ].get.summary || null}`
        )
      }
      ...
      if (utils.isValidChinese(checkApiDescData[key].post.summary)) {
        ...
        errGroup.lev1['0x20'].push(
          `接口[ POST  -> ${key} ]中必须有中文介绍描述  --->  ${checkApiDescData[
            key
          ].post.summary || null}`
        )
        checkArr.push(false)
...
// 接口入参描述检测
async function checkApiParamsFunc (
  checkApiParamsData,
    ...
    if (checkApiParamsData[key].get && checkApiParamsData[key].get.parameters) {
      ...
      checkApiParamsData[key].get.parameters.map((item, index) => {
        if (item.name && item.name.trim().length > 0) {
          ...
          checkArr.push(false)
          errGroup.lev1['0x30'].push(
            `接口[ GET  -> ${key} ]参数${index +
              1}参数名不能为空  --->  ${item.name || null}`
          )
        }
        ...
        if (item.description && item.description.trim().length > 0) {
          ...
          errGroup.lev1['0x30'].push(
            `接口[ GET  -> ${key} ]参数${index +
              1}参数描述不能为空  --->  ${item.description || null}`
          )
        }
        ...
        if (item.schema && item.schema.$ref) {
          ...
          for (let k in eachKey) {
            ...
            if (eachKey[k].type && eachKey[k].type.length > 0) {
              ...
              checkArr.push(false)
              errGroup.lev1['0x30'].push(
                `接口[ POST  -> ${key} ]中参数[ ${k} ]的类型不能为空  --->  ${eachKey[
                  k
                ].type || null}`
              )
            }
            if (
              eachKey[k].description &&
              eachKey[k].description.trim().length > 0
            ) {
              ...
              checkArr.push(false)
              errGroup.lev1['0x30'].push(
                `接口[ POST -> ${key} ]中参数[ ${k} ]的描述不能为空---> ${eachKey[
                  k
                ].description || null} `
              )
            }
...
async function checkResFunc (checkApiResData, checkDefinitionsData, errGroup) {
  ...
  for (let key in checkApiResData) {
    if (checkApiResData[key].get) {
      // console.log('\x1b[37m', `➢  接口[ GET -> ${key} ]`, '\x1b[0m')
      if (
        !checkApiResData[key].get.responses['200'].schema ||
        !checkApiResData[key].get.responses['200'].schema.$ref
      ) {
        checkArr.push(false)
        errGroup.lev1['0x30'].push(`接口[ ${key} ]返回不能为空---> ${null} `)
      } else {
        let keyWord = checkApiResData[key].get.responses[
          '200'
        ].schema.$ref.split('/')[2]
        let eachKey = checkDefinitionsData[keyWord].properties
        if (eachKey.code) {
        ...
        if (resKeyword) {
          ...
          for (let k in eachKeyRes) {
            if (k && k.length > 0) {
              ...
              checkArr.push(false)
              errGroup.lev1['0x40'].push(
                `接口[ GET  -> ${key} ]返回参数名不能为空  --->  ${k || null}`
              )
            }
            if (eachKeyRes[k].type && eachKeyRes[k].type.length > 0) {
              ...
              checkArr.push(false)
              errGroup.lev1['0x40'].push(
                `接口[ GET  -> ${key} ]中返回参数[ ${k} ]的类型不能为空  --->  ${eachKeyRes[
                  k
                ].type || null}`
              )
            }
            if (
              eachKeyRes[k].description &&
              eachKeyRes[k].description.trim().length > 0
            ) {
              ...
              checkArr.push(false)
              errGroup.lev1['0x40'].push(
                `接口[ GET -> ${key} ]中返回参数[ ${k} ]的描述不能为空---> ${eachKeyRes[
                  k
                ].description || null} `
              )
            }
      ...
      // console.log('\x1b[37m', `➢  接口[ POST -> ${key} ]`, '\x1b[0m')
      if (
        !checkApiResData[key].post.responses['200'].schema ||
        !checkApiResData[key].post.responses['200'].schema.$ref
      ) {
        checkArr.push(false)
        errGroup.lev1['0x30'].push(`接口[ ${key} ]返回不能为空 ${null} `)
      } else {
        let keyWord = checkApiResData[key].post.responses[
          '200'
        ].schema.$ref.split('/')[2]
        let eachKey = checkDefinitionsData[keyWord].properties
        if (eachKey.code) {
        ...
        if (resKeyword) {
          ...
          for (let k in eachKeyRes) {
            ...
            if (eachKeyRes[k].type && eachKeyRes[k].type.length > 0) {
              ...
              checkArr.push(false)
              errGroup.lev1['0x40'].push(
                `接口[ POST  -> ${key} ]中返回参数[ ${k} ]的类型不能为空  --->  ${eachKeyRes[
                  k
                ].type || null}`
              )
            }
            if (
              ...
              checkArr.push(false)
              errGroup.lev1['0x40'].push(
                `接口[ POST -> ${key} ]中返回参数[ ${k} ]的描述不能为空---> ${eachKeyRes[
                  k
                ].description || null} `
              )
            }
