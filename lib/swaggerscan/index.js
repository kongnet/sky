 ...
 * @Last Modified time: 2019-04-19 17:23:39
 * @Desc: 用于自动检测接口定义规范
 * ✔ ✘ ❀ ㋡ ☹ ➢
 */
...
// const docsUrl = 'http://msr.jiatuidev.com/ai-article/v2/api-docs'
// checkDocs(errGroup, docsUrl)
...
async function scan (file) {
  try {
    ...
    conf = file ? require(path.join(process.cwd(), file)).swaggerscan : require('./config').swaggerscan.url
  } catch (e) {
...
async function checkDocs (errGroup, url) {
  ...
  if (url.indexOf('http') !== -1) {
    ...
    try {
      let jsonDataLocal = require(url)
      checkJson = jsonDataLocal
    } catch (e) {
  ...
  if (!checkJson) {
    errGroup['lev0'] = 'swagger文件加载失败：无法获取目标url，请检查url是否正确'
    return
  } else if (typeof checkJson === 'string') {
    errGroup['lev0'] = 'swagger文件加载失败：目标无法转为JSON，请检查JSON格式是否正确'
    return
  } else if (!checkJson.swagger) {
    errGroup['lev0'] = 'swagger文件加载失败：目标未指明swagger版本，请检查是否swagger的JSON配置'
    return
  }
...
// 输出错误JSON
function checkExportJson (errGroup) {
  console.log('\x1b[41;37m', '   输出包含错误的Json:', '\x1b[0m')
  console.log('')
  $.dir(errGroup)
  console.log('')
}
...
async function checkServFunc (checkServData, errGroup) {
  ...
  if (checkServData) {
    // 服务中文名检测
    if (checkServData.title && checkServData.title.length > 0) {
    ...
    // 服务中文描述检测
    if (checkServData.description && checkServData.description.length > 0) {
    ...
    // 服务联系人姓名检测
    if (checkServData.contact && checkServData.contact.name && checkServData.contact.name.length > 0) {
    ...
    // 服务联系人邮箱检测
    if (checkServData.contact && checkServData.contact.email && checkServData.contact.email.length > 0) {
      ...
      checkApiParamsData[key]['post']['parameters'].map((item, index) => {
        if (item.schema) {
          ...
          for (let k in eachKey) {
            if (k && k.length > 0) {
              console.log('\x1b[32m', `✔      存在参数名  --->  ` + `${k || null}`, '\x1b[0m')
              checkArr.push(true)
            } else {
            ...
            if (eachKey[k].description && eachKey[k].description.length > 0) {
      ...
      if (!checkApiResData[key]['get']['responses']['200']['schema'] || !checkApiResData[key]['get']['responses']['200']['schema']['$ref']) {
        ...
        if (resKeyword) {
          ...
          for (let k in eachKeyRes) {
            if (k && k.length > 0) {
              console.log('\x1b[37m', `➫      返回参数名  --->  ` + `${k || null}`, '\x1b[0m')
              checkArr.push(true)
            } else {
            ...
            if (eachKeyRes[k].description && eachKeyRes[k].description.length > 0) {
