...
function initAddrList (n) {
  ...
  console.log(
    $.c.y('wallet-gen.json'),
    `has been created pls take care of them!`
  )
}

function initEkList (easyKey, easyIv) {
  if (!fs.existsSync('./wallet-gen.json')) {
    console.log(
      $.c.y('wallet-gen.json'),
  ...
  console.log(
    ...
    `has been created pls take care of`,
    $.c.y('wallet-gen.json')
  )
  let outWrapStr = JSON.stringify({ key, iv, addrList })
  if (easyKey && easyIv) {
    let cryptoObj = new $.Crypto()
    outWrapStr = cryptoObj.easyAES.encrypt(outWrapStr, easyKey, easyIv)
  }
  console.log(
  ...
  clipboard.writeSync(outWrapStr)
}
