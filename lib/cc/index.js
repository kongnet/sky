...
function installCcjt () {
  try {
    ...
    let isEslintIgnore = fs.existsSync(path.join(process.cwd(), '.eslintignore'))
    let isGitIgnore = fs.existsSync(path.join(process.cwd(), '.gitignore'))
    let eslintFile = isEslintIgnore ? fs.readFileSync(path.join(process.cwd(), '.eslintignore')).toString() : ''
    let gitFile = isGitIgnore ? fs.readFileSync(path.join(process.cwd(), '.gitignore')).toString() : ''
    let outputFileAry = [eslintFile.split('\n'), gitFile.split('\n')].flatten().unique().filter(Boolean)
  ...
  o.forEach(it => {
    ...
    if (it.messages.length > 0) {
      it.messages.forEach(msgIt => {
        errType[msgIt.ruleId] = errType[msgIt.ruleId] ? (errType[msgIt.ruleId] + 1) : 1
...
function runJtFrontCheck (isFixing = false, isStat = false) {
  try {
    ...
    console.log('运行JT 前端检测，请耐心等候... jt强制规则', ccjt.jtStat.errorCount, 'jt警告规则', ccjt.jtStat.warnCount)
    jtRule.resolvePluginsRelativeTo = path.join(__dirname, '../../node_modules')
    jtRule.useEslintrc = false // 忽略 .eslintrc文件
    ...
    const report = cli.executeOnFiles(['.'])
    const formatHtml = cli.getFormatter(path.join(__dirname, '../../node_modules', 'eslint-formatter-html-extended'))
    const formatDefault = cli.getFormatter(isStat ? 'json' : '')
    CLIEngine.outputFixes(report)
    console.log(isStat ? eslintJson(report.results) : formatDefault(report.results))
    // fs.writeFileSync('jtFrontCheck.html', formatHtml(report.results))
    console.log($.c.y('jtFrontCheck.html 成功生成!'), '本次扫描耗时', Date.now() - t, 'ms')
  } catch (e) {
    if (e.toString().includes('No files matching')) {
      console.log($.c.c('您的项目没有任何问题，厉害'))
    } else {
...
function runJtFrontEasyCheck () {
  try {
    ...
    jtRule.rules['max-statements'] = [1, { 'max': 100 }, { 'ignoreTopLevelFunctions': true }]
    jtRule.rules['max-lines-per-function'] = [1, 100]

    console.log('运行JT 前端规模简易检测，请耐心等候... jt强制规则', ccjt.jtStat.errorCount, 'jt警告规则', ccjt.jtStat.warnCount)
    jtRule.resolvePluginsRelativeTo = path.join(__dirname, '../../node_modules')
    jtRule.useEslintrc = false // 忽略 .eslintrc文件
    jtRule.extensions = ['.js', '.html', '.htm', '.vue']
    ...
    console.log($.c.y('jtFrontEasyCheck.html 成功生成!'), '本次扫描耗时', Date.now() - t, 'ms')
  } catch (e) {
    if (e.toString().includes('No files matching')) {
      console.log($.c.c('您的项目没有任何问题，厉害'))
    } else {
...
function listTodoLabel () {
  try {
    ...
    const labelAry = []
    jtRule.rules['no-warning-comments'] = [1, { 'terms': todoLabel, 'location': 'anywhere' }]
    ...
    report.results.forEach(it => {
      ...
      it.messages.forEach(msgIt => {
        ...
        labelAry.push({
          '文件': it.filePath.replace(process.cwd(), '') + ':' + msgIt.line + ':' + msgIt.column,
          '标记': RegExp.$1,
...
function checkVariable () {
  try {
    ...
    const jtRule = ccjt.cc
    const labelAry = []
    jtRule.rules['id-match'] = [1, '[!]+', { 'onlyDeclarations': true, 'properties': false }]
